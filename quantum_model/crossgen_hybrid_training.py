...
def default_data_root(project_root: Path) -> Path:
    return project_root / "ariel-ml-dataset"


def default_output_dir(project_root: Path) -> Path:
    return project_root / "outputs" / "model_quant_sketch_adc"
...
class TrainingConfig:
    project_root: str = "."
    data_root: str = "ariel-ml-dataset"
    output_dir: str = "outputs/model_quant_sketch_adc"
    seed: int = 42
...
def load_adc_dataset(data_root: Path) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    aux_path = data_root / "TrainingData" / "AuxillaryTable.csv"
    gt_path = data_root / "TrainingData" / "Ground Truth Package" / "FM_Parameter_Table.csv"
    spectra_path = data_root / "TrainingData" / "SpectralData.hdf5"

    aux_df = pd.read_csv(aux_path)
    gt_df = pd.read_csv(gt_path)

    labels = aux_df.merge(gt_df[["planet_ID"] + TARGET_COLS], on="planet_ID", how="inner")
    labels = labels.reset_index(drop=True)
    n_samples = len(labels)

    with h5py.File(spectra_path, "r") as handle:
        first_key = list(handle.keys())[0]
        n_bins = handle[first_key]["instrument_wlgrid"].shape[0]
        ...
        for i, pid in enumerate(labels["planet_ID"].values):
            h5_key = f"Planet_{pid}"
            grp = handle[h5_key]
            noisy_spectra[i] = grp["instrument_spectrum"][:]
            noise_arr[i] = grp["instrument_noise"][:]

    noise_mean = noise_arr.mean(axis=1)
    labels["log10_noise_mean"] = np.log10(np.clip(noise_mean, 1e-10, None))

    return labels, noisy_spectra, noise_arr, wavelength_um
...
def make_split_tensors(
    ...
    return SplitTensors(
        sample_ids=labels["planet_ID"].to_numpy(dtype="U32"),
...
def prepare_data(config: TrainingConfig) -> PreparedData:
    labels, noisy_spectra, noise_arr, wavelength_um = load_adc_dataset(config.resolved_data_root())
    ...
    train_val_indices, test_indices = train_test_split(
        all_indices, test_size=0.10, random_state=config.seed, shuffle=True,
    )
    inner_train_indices, inner_val_indices = train_test_split(
        train_val_indices, test_size=config.internal_val_fraction, random_state=config.seed, shuffle=True,
    )
    inner_train_indices = np.sort(inner_train_indices)
    inner_val_indices = np.sort(inner_val_indices)
