...
def prepare_garnet_run(
    ...
    backend = resolve_iqm_backend(config.backend_mode, config.backend_url, config.backend_alias, config.backend_token)
    chosen_layout = (
        LayoutSelection(
...
def run_mock_evaluation(run: PreparedGarnetRun, backend: Optional[Any] = None) -> np.ndarray:
    ...
    active_backend = backend or resolve_iqm_backend("fake", run.config.backend_url, run.config.backend_alias, run.config.backend_token)
