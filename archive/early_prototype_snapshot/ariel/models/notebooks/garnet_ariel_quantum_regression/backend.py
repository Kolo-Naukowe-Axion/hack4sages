...
def import_iqm() -> tuple[Any, Any]:
    ...
    try:
        ...
        try:
            ...
            raise ImportError("The installed IQM Qiskit package does not expose IQMFakeGarnet.") from exc
...
def resolve_iqm_backend(
    ...
    resolved_token = backend_token or os.environ.get("IQM_TOKEN")
    if resolved_token:
        auth_kwargs["token"] = resolved_token
    if normalized_mode == "mock":
        provider = IQMProvider(backend_url, quantum_computer=f"{backend_alias}:mock", **auth_kwargs)
    ...
    if normalized_mode == "real":
        if "token" not in auth_kwargs:
            raise RuntimeError("Real IQM backend access requires an IQM token. Set IQM_TOKEN or pass backend_token.")
        provider = IQMProvider(backend_url, quantum_computer=backend_alias, **auth_kwargs)
        return provider.get_backend()
