...
def resolve_training_device(config: TrainingConfig) -> torch.device:
    if config.training_device is not None:
        requested = str(config.training_device).lower()
        if requested == "cuda":
            if not torch.cuda.is_available():
                raise RuntimeError("training_device=cuda requires CUDA, but CUDA is unavailable.")
            return torch.device("cuda")
        if requested == "mps":
            if getattr(torch.backends, "mps", None) is None or not torch.backends.mps.is_available():
                raise RuntimeError("training_device=mps requires MPS, but MPS is unavailable.")
            if not config.classical_only:
                raise RuntimeError(
                    "training_device=mps is only supported for classical_only runs. "
                    "Hybrid quantum runs should use cpu or cuda."
                )
            return torch.device("mps")
        if requested == "cpu":
            return torch.device("cpu")
        raise ValueError(f"Unsupported training_device={config.training_device!r}. Use cpu, cuda, or mps.")
...
def maybe_initialize_from_checkpoint(model: HybridArielRegressor, config: TrainingConfig) -> None:
    ...
    source_classical_only = bool(checkpoint_config.get("classical_only", False))
    state_dict = payload.get("model_state_dict", payload)
    model_state = model.state_dict()
    filtered_state: dict[str, torch.Tensor] = {}
    skipped_shape_keys: list[str] = []
    for key, value in state_dict.items():
        if key == "quantum_gate" and source_classical_only and not config.classical_only:
            continue
...
def train_model(config: TrainingConfig, data: PreparedData, device: torch.device, output_dir: Optional[Path] = None) -> dict[str, Any]:
    model = build_model(
        ModelConfig(
            ...
            quantum_gate_init=config.quantum_gate_init,
    ...
    if config.max_runtime_seconds is not None and float(config.max_runtime_seconds) > 0:
        runtime_deadline = training_started + float(config.max_runtime_seconds)
    ...
    if runtime_deadline is not None:
        print(f"Max runtime seconds: {float(config.max_runtime_seconds):.1f}", flush=True)

    for epoch in range(config.max_epochs):
        if runtime_deadline is not None and time.perf_counter() >= runtime_deadline:
            print("Runtime budget reached before starting the next epoch.", flush=True)
            status = "stopped_runtime"
            break
        model.train()
        ...
        for batch_index, indices in enumerate(
            ...
            if runtime_deadline is not None and time.perf_counter() >= runtime_deadline:
                print(
                    f"Runtime budget reached during epoch {epoch + 1}; evaluating current model state.",
                    flush=True,
                )
        ...
        improved = val_metrics["rmse_mean"] < best_val_rmse
        will_stop_early = (not improved) and (patience_left - 1 <= 0)
        epoch_log_interval = max(1, int(config.log_every_epochs))
        should_log_epoch = (
            epoch == 0
            or (epoch + 1) % epoch_log_interval == 0
            or runtime_stop_requested
            or will_stop_early
...
def run_training_experiment(config: Optional[TrainingConfig] = None) -> dict[str, Any]:
    ...
    summary = {
        ...
        "max_runtime_seconds": None if cfg.max_runtime_seconds is None else float(cfg.max_runtime_seconds),
