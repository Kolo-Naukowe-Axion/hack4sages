...
from models.sbi_ariel_crossgen.training import (
    append_jsonl,
    build_eval_loader,
    build_train_loader,
    close_dataloader,
...
def train_model(
    ...
    max_runtime_seconds = train_loader_settings.get("max_runtime_seconds")
    patience = int(train_loader_settings["patience"])
    status = "continue"
    training_started = time.perf_counter()
    runtime_deadline = None
    if max_runtime_seconds is not None and float(max_runtime_seconds) > 0:
        runtime_deadline = training_started + float(max_runtime_seconds)
    ...
    if runtime_deadline is not None:
        print(f"Max runtime seconds: {float(max_runtime_seconds):.1f}", flush=True)
    ...
    while status == "continue" and state.epoch <= max_epochs:
        ...
        try:
            for batch_idx, batch in enumerate(train_loader, start=state.batch_in_epoch + 1):
                ...
                if runtime_deadline is not None and time.perf_counter() >= runtime_deadline:
                    ...
                    print(
                        f"Runtime budget reached during epoch {state.epoch}; evaluating current model state.",
                        flush=True,
                    )
                    break
        finally:
        ...
        if status == "stopped_max_steps":
            break
        if state.epoch_loss_count == 0:
            status = "stopped_runtime" if runtime_stop_requested else status
            break
    ...
    evaluation_outputs = (
        ...
        if (status in {"completed", "stopped_early", "stopped_runtime"} and bool(run_final_evaluation))
        else {}
    )
