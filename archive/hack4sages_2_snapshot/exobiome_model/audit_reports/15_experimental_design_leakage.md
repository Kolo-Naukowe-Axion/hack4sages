...
| `exobiome_model/outputs/model_crossgen_rebinned` | Mar 12 ~01:16 | 12 | 5 aux | 3.463 |

The final `exobiome_model` run is an exact copy of `quantum_model_crossgen` (identical `run_summary.json` values: best_epoch=29, best_inner_val_loss=0.2999, test_rmse_mean=3.4631). The `_fe` variant (with 2 extra features: `rp_rs_ratio`, `scale_height_proxy`) had worse test RMSE (3.538 vs 3.463) and was abandoned.
...
All subsequent runs (`quantum_model_crossgen`, `quantum_model_crossgen_fe`, `exobiome_model`) switched to:
```python
