...
def default_output_dir(project_root: Path) -> Path:
    return project_root / "outputs" / "model_crossgen_rebinned_fe"
...
class TrainingConfig:
    project_root: str = "."
    data_root: str = ""
    output_dir: str = "outputs/model_crossgen_rebinned_fe"
    seed: int = 42
...
def load_crossgen_dataset(data_root: Path) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    ...
    labels["rp_rs_ratio"] = labels["planet_radius_rjup"] / labels["star_radius_rsun"].clip(lower=1e-10)
    labels["scale_height_proxy"] = labels["temperature_k"] / (10.0 ** labels["log_g_cgs"]).clip(lower=1e-10)
