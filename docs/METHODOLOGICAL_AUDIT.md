...
| 3 | Model przewiduje **5 z 7** parametrów swojego benchmarku — pomija **temperaturę i promień**. Pominięta temperatura nie jest słabo sprzężonym dodatkiem, a **dominującą degeneracją** tego, co model raportuje: sprzężenie T–gaz w posteriorze referencyjnym wynosi **0,221** wobec **0,053** gaz–gaz, czyli **4,14× silniej** (663 planety), a nachylenie posterioru to **0,352 dex/100 K**. Na crossgenie temperatura nie jest ani przewidywana, ani odzyskiwalna z tabeli pomocniczej (`corr(T_eq z aux, T prawdziwe) = 0,0003`), choć sama rozciąga skalę wysokości **3,60×** w zakresie 500–1800 K. Raportowane obfitości nie mają więc uchwytu na swój główny confounder. | `audit/a15` |
...
**Zastrzeżenie o budżecie treningowym, konieczne przy czytaniu tej tabeli.** Szczeble drabiny są
dopasowane na **12 000 wierszach** (`a26 --n-train`, domyślnie 12000), i to na **prefiksie** listy id
(`ids[:limit]`, `a26:315-316`), nie na losowej podpróbce. ExoBiome uczył się na **całym** splicie
treningowym ADC, czyli 33 138 wierszach. Baseline'y są więc oceniane przy **2,8× mniejszym** budżecie
danych niż model, obok którego stoją. Konsekwencja jest jednostronna i trzeba ją nazwać: przy pełnym
budżecie ridge i GBM byłyby **nie gorsze**, więc przewaga ExoBiome nad nimi jest **górnym**
oszacowaniem, a nie dolnym. To ta sama wada niedopasowanego budżetu, którą audyt zarzuca projektowi
w K3 i K5 — tutaj obciąża **nasze własne** liczby i dlatego stoi w tekście, a nie w przypisie.
...
zweryfikowaną wiersz po wierszu. Dwa zastrzeżenia, oba istotne i oba trzeba czytać razem z tą liczbą:

- **To skill na mRMSE, a mRMSE nie jest metryką tego challenge'u** (0,8·KS na siedmiu marginalnych +
  0,2·widmowa — patrz K10). „Najlepszy wynik" znaczy więc „najlepszy na skali, której benchmark nie
  używa", a nie „najlepszy w konkursie".
- **Wiersz baseline'u organizatorów nie stoi na tej samej osi co pozostałe.** Liczba `0.6500` pochodzi
  z `models/adc_baseline/cnn_metrics.json` i ma pokrycie w wagach oraz kodzie (`a13`: `backed_summary`,
  `predictions: false`), ale **na których wierszach powstała, nie da się z repozytorium ustalić**:
  `run_baseline.py:32` ustawia `N = 5000`, a cięcie `iloc[:N]` pada w `:66` i `:72`; linia 79 dzieli `np.random.rand(len(spectra)) < threshold` (`threshold = 0.8`, `:31`) **przed**
  `set_random_seed` w linii 127, a ewaluacja idzie na osobną ścieżkę `--test`. Skill w tym wierszu jest
  policzony wobec podłogi z naszych 4143 wierszy przy założeniu, że liczba ich dotyczy — założenia tego
  nie zweryfikowano. Wiersz zostawiamy jako orientację, nie jako porównanie.
...
z trzech klas uczących się nie bije stałej. Uwaga na liczenie: **te cztery pomiary nie są czterema
niezależnymi modelami**, ale nie z tego powodu, że quantum i noquant to jeden model z przełącznikiem —
`a11` pokazuje, że to **dwa osobne przebiegi treningowe różniące się w 19 polach konfiguracji**
(`batch_size` 128/256, `classical_lr` 5e-5/1e-3, `max_epochs` 20/60, `init_checkpoint_path`
obecny/`null`, `quantum_backbone_freeze_epochs` 4/0 i dalej), z czego **tylko jedno** (`quantum_device`)
jest zadeklarowanym czynnikiem różnicującym. Klas uczących się są zatem trzy — sieć, regresja liniowa
z karą, drzewa wzmacniane gradientowo — ale różnica quantum vs noquant **nie jest przypisywalna gałęzi
kwantowej** i nie wolno jej tak czytać (`a11.interpretation`). W tym akapicie liczy się wyłącznie
**wspólny znak** wszystkich czterech pomiarów, a nie ich kolejność ani odstępy.
...
generatorami. Tańszy krok, którego audyt nie wykonał: ridge/GBM uczone **i** oceniane in-domain na
POSEIDON (cross-walidacja na tych samych 685 wierszach, infrastruktura już istnieje w `a26:354-383` —
brakuje jednej konfiguracji wywołania). Ujemny skill i tam zamknąłby hipotezę „modele nie przenoszą się
między generatorami" bez odwoływania się do fizyki widma. Rozdziela to jednak dopiero pomiar
...
4. **Determinizm modelu.** Dwa wywołania na tym samym checkpoincie dają `max |p₁ − p₂| = 0.000e+00`.
   ...
   (`a24:192` — seed, `a24:198` — losowanie), są już zaseedowane po stronie audytu i potwierdzone bit-identyczne między
   przebiegami.
...
Ograniczenie prowenienacji, warte zapisania: sumy kontrolne wejść są liczone na **pierwszych 64 MiB** pliku (`audit_lib.py:59-69`), co odcisk sygnalizuje w kluczu (`…-first67108864B`). Dla `data/TauREx set/spectra.h5` (68,7 MB) odcisk **nie identyfikuje całego pliku**, choć checki czytają wszystkie 685 + 41 423 wiersze.
...
**Zakres tej walidacji, podany dokładnie.** `a27` pokrywa **jeden** tor: ścieżkę **czystą** ExoBiome na
splicie `holdout`, przy skalach 1.0 i 0.5, wobec `mac_holdout_predictions.csv` (`a27:88-97,116`). **Nie
pokrywa** trzech innych, na których również stoją cytowane liczby: (i) ścieżki **zaszumionej**
(`audit_lib.py:373-375`), z której pochodzą `0.966512 ± 0.006572` i `degradation_factor = 3.236` w K3 —
dla niej nie ma w repo referencji do porównania; (ii) całej ścieżki `a15` (crossgen, HistGBM,
`Tracedata.hdf5`), na której stoi K9; (iii) niezależnego kodu cech w `a26` (`a26:309-383`). Margines 11×
jest realny i zmierzony, ale dotyczy toru (i) w wersji czystej — nie jest walidacją całego harnessu.
Dodatkowo: pole `pass_criterion` mówi „przy `quantum_scale=1.0`", a implementacja przechodzi, gdy zgodna
jest **którakolwiek** ze skal (`a27:132-135`, `any(...)`); w tym przebiegu zgodna jest dokładnie 1.0, więc
liczby są w porządku, ale zapisane kryterium nie jest tym, które kod wykonuje.

**Skąd tolerancja `1e-4` i wobec czego naprawdę jest marginesem.** Wartość jest **przyjęta, nie
wyprowadzona** (`a27:46`), i trzeba ją czytać wobec właściwego poziomu, bo `a27` mierzy dwie zupełnie
różne rzeczy. Referencja `mac_cpu` to **ta sama ścieżka kodu** (CPU / `lightning.qubit` / fp32,
`references.mac_cpu.meta.same_code_path = true`), więc tam oczekiwana rozbieżność to szum
zaokrągleń fp32 — rzędu 10⁻⁷–10⁻⁶ — i wobec niego `1e-4` jest progiem z zapasem. Referencja `gpu_amp`
(zacommitowane `holdout_predictions.csv`, CUDA + bfloat16) to **inna** ścieżka numeryczna i jej
rozbieżność per komórka jest o dwa rzędy większa, niż sugerowałaby różnica agregatów: `a27` mierzy ją
sam i wychodzi `mean_abs_diff = 0.0194`, `p99 = 0.206`, `max = 0.961`, `rows_over_tolerance = 4143 z 4143`.
Poziom „~1e-3" dotyczy więc **różnicy agregatu mRMSE** (0.298693 vs 0.299376 = 6,8e-4), a nie
pojedynczych komórek — i to rozróżnienie jest tu istotne, bo:

- „margines 11×" jest marginesem **wyłącznie na torze `mac_cpu`**, czyli dla rekonstrukcji wobec
  re-ewaluacji na Macu;
- **zacommitowany plik predykcji z runu CUDA nie jest przez rekonstrukcję odtwarzany na żadnym wierszu**,
  i to jest własność ścieżki numerycznej (AMP), nie błąd rekonstrukcji — ale znaczy, że `a27` nie
  waliduje artefaktu, który projekt opublikował, tylko jego re-ewaluację na CPU.

| wielkość | referencja `mac_cpu` (ta sama ścieżka kodu) | referencja `gpu_amp` (CUDA + bf16) |
|---|---:|---:|
| kryterium PASS (tolerancja) | 1,0e-4 | 1,0e-4 |
| największa różnica na 20 715 komórkach | **8,9e-6** | 9,6e-1 |
| p99 różnic | 1,5e-6 | 2,1e-1 |
| średnia różnic | 2,7e-7 | 1,9e-2 |
| wierszy powyżej tolerancji | **0 z 4143** | 4143 z 4143 |

Na torze `mac_cpu` najgorsza komórka leży **jedenaście razy poniżej progu**, przy którym check by upadł —
a gdyby rekonstrukcja była błędna, rozbieżności byłyby rzędu samych mierzonych efektów (0,003–0,007),
nie 10⁻⁶. Status `PASS` liczony jest **tylko** z kolumny `mac_cpu` (`a27:132-135`, `strict =
payload["references"]["mac_cpu"]`), więc kolumna `gpu_amp` jest raportowana, nie punktowana.
...
dwa niezależne uruchomienia `a03` (`20260729` i `20260729_v2`) dają **0 różnic na wszystkich liczbowych polach payloadu**. Reszta harnessu była deterministyczna
...
| K10(b) | Oficjalny baseline ADC2023 jest w repo (`models/adc_baseline/`), używa MC-dropout i produkuje rozkład|  lektura + `ucl-exoplanets/ADC2023-baseline` | PROVEN |
...
| Suite 7 testów: kierunek prawdopodobny, liczby zepsute | czytanie kodu | — |
...
**Mechanizm maskujący w preprocessingu.** Zabezpieczenie, które tego nie łapie: `data/crossgen_biosignatures/validate_dataset.py:78-90` — i nie jest to jedyne miejsce, które tego nie złapało. Po prostu **pipeline modelu nie mógł tego zgłosić, bo usuwa dokładnie tę
...
Literatura. arXiv:1409.2312 (Waldmann i in. 2014, TauREx I), arXiv:1912.07759v2 (Al-Refaie et. al 2021,Taurex III) jest referencją dla konfiguracji
...
dodatniość, zakresy priorów i prewalencję (`:87-90`) — nie sprawdza, czy widmo zależy od długości fali.
...
`v1.3.2` (`7077f1036e1aa46008efa262ba645dfe7ec1fc7e`) daje **bit-identyczne wielkości geometrii** —
i tylko tyle wolno z rekordów wyczytać. Bit-identyczność dotyczy pól obecnych w **obu** rekordach
(`r_min_km`, `r_max_km`, `vertical_extent_km`, `extent_over_Rp`, `T_profile_unique`, `X_shape`,
`X_per_species_median_log10`). Natomiast **test nasycenia, skala wysokości, promień referencyjny i tło
H₂/He istnieją wyłącznie w rekordzie 1.3.2**: `reports/audit/20260727/d01_poseidon_diagnosis.json`
(1.4.0, `status: INFO`) nie zawiera kluczy `saturation_test`, `scale_height_km`, `r_at_P_ref_km`,
`background_H2_fraction` ani `verdict_opacity`, a jego `verdict_H1` kończy się innym zdaniem
(„the cause is elsewhere, go to Stage 2" wobec „the cause is radiative — see saturation_test"). To były
więc **dwie wersje kodu checku**, nie jeden pomiar na dwóch wersjach biblioteki. Werdykt `SATURATED` ma
zatem **jedną** ścieżkę dowodową, na 1.3.2 — i to jest wersja generująca zbiór, czyli ta właściwa, ale
nie jest to potwierdzenie cross-wersyjne. Instalacja 1.3.2 wymagała dwóch dodatkowych zależności systemowych, obie
...
`pysynphot`, importowany przez `POSEIDON.stellar`). Przypięcie 1.3.2 dla Etapu 2 (72,1 GB)
...
| wartość oczekiwana | Wszystkie liczby geometrii i testu rozróżniającego cytowane w „Wyjaśnieniu" wyżej pochodzą z payloadu **1.3.2** (wersja generująca). Na 1.4.0 potwierdzona bit-identycznie jest **wyłącznie geometria** — `saturation_test` w rekordzie 1.4.0 nie istnieje, więc nie jest to pomiar cross-wersyjny. Dodatkowo: `input_data_root = null`, `opacity_files = []` (blocker Etapu 0 — Etap 2 potrzebuje zenodo 16107813, 72,1 GB); `grids_bit_identical = true`; `verdict_H1` niepotwierdzona. Rdzeń dowodu, `saturation_test`: `recorded_depth = 0,016985462978482246` (`n_unique_bins = 1`), `R_s_km = 562 182,28`, `implied_radius_km = 73 268,254`; ilorazy `atmosphere_bottom = 0,9012`, `R_p_ref = 0,9123`, `atmosphere_top = 0,9995` przy `tolerance = 5e-3` → `matching_candidates = [atmosphere_top]`, `verdict_opacity = SATURATED` |
...
modelu zwycięzców na **79 z 300 epok**: `a05` (`runs.adc_winner_nsf`) notuje `lr_collapsed = true`
przy `lr_anneal_factor = 256`, `stopping_metric = "val NLL"` wobec
`selection_metric = "mRMSE on 256 rows x 16 samples every 10 epochs"`, a przy tym
`comparison_metric_still_improving = true`. To ostatnie pole trzeba czytać dokładnie: `a05:126` ustawia
je jako `best_i >= len(vals) - IMPROVING_TAIL_MEASUREMENTS`, gdzie stała wynosi 2 (`a05:48`) — znaczy
więc „minimum wypadło w **dwóch ostatnich** pomiarach", a **nie** „metryka jeszcze rosła". Pomiary są
co 10 epok i wyglądają tak: `0.6299 / 0.6122 / 0.5736 / 0.5799 / 0.5629 / 0.5450 / 0.5657` — minimum
na pomiarze **6 z 7** (epoka 60), a jedyny pomiar po nim jest o **3,8 % gorszy** (epoka 70). Zarzut
jest więc taki: model porównawczy zatrzymano metryką, która nie jest metryką porównania, i to tak blisko
minimum tej drugiej, że **zbieżności na niej nie da się orzec** — a nie taki, że metryka pewnie by
jeszcze spadała.
...
**Ile szumu jest w widmach ADC?** Dokumentacja zbioru odpowiada na to wprost: widma są idealne, szum nie jest do nich wliczony, a tablica σ jest dostarczana osobno (arXiv:2309.09337, §1.2 Noise; potwierdzone w publicznym repozytorium AstroAI-CfA). Nasz własny test rozrzutu bin-do-binu (a09, status FAIL) jest zgodny z kierunkiem „mniej niż pełna σ", ale nie rozstrzyga tego niezależnie — brakuje kalibracji na tym samym binowaniu. Różnica jest przy tym **głębsza niż samo binowanie**: dla ramion tau σ jest **skalarem na wiersz** rozdmuchanym na zahardkodowane 218 binów (`a09:71`), a dla ADC jest **per-binowa** na 52 binach (`a09:83-88`, pole `adc_sigma_is_per_bin = true`). Próg `0.8744` = `0,80 × 1,0930` skalibrowano na tau i przeniesiono na ADC (`a09:34,100-103`). Sam ułamek 0,80 jest w kodzie uzasadniony i zadeklarowany z góry, wraz z analizą czułości (`a09:30-34`: „gdyby prog wynosil 0.70, ADC przeszloby — wiec wybor ulamka MA znaczenie i musi byc jawny"), a pochodzenie progu wchodzi do payloadu (`threshold.basis`, `threshold.calibration_fraction`). **Nieodnotowane jest natomiast samo przeniesienie**: nic w kodzie ani w payloadzie nie mówi, że próg skalibrowany na 218 binach ze skalarnym σ stosuje się do 52 binów z σ per-bin. Osobno, i to jest luka w harnessie tej samej klasy co przy `a27`: pole `pass_criterion` deklaruje „R statistic distribution centred at **≥ 1**", a implementacja porównuje medianę z progiem **0,8744** (`a09:100,109`). Werdykt się od tego nie zmienia (mediana ADC = 0,757 leży pod obiema poprzeczkami), ale zapisane kryterium nie jest tym, które kod wykonuje.
...
| ADC `instrument_spectrum` (pytanie otwarte) | 0.319 | **0.757** | 2.108 | 0.627 |
...
|---|---|---|
| grep | `nn.Parameter(` + `torch.zeros(len(TARGET_COLUMNS), dtype=torch.float32, device=classical_device)` — wywołanie łamane na trzy linie (`model.py:260-262`) | `randn`/`ones` w miejscu `torch.zeros`|
| T1 | 0,046293 | inna liczba |
| T2 | **False** — CO₂ `+`, cztery pozostałe `−` | `True` `mean\|tanh\|` byłoby adekwatnym podsumowaniem, zarzut osłabiony |
| T3 | mnożnik `\|tanh(gate)\|·0,5 ∈ [0,0178; 0,0292]` | wartość mnożnika rzędu 1 — wtedy korekta kwantowa miałaby wpływ porównywalny z głową klasyczną |
| T4 | ~12 426 kroków przy `lr = 2e-4` | `max \|raw gate\|` rosnące monotonicznie do końca treningu przy tym budżecie kroków — wtedy zero-init nie byłby wąskim gardłem, a bramka po prostu potrzebowałaby więcej epok |
...
To nie jest podpróbka losowa i audyt sam to zmierzył: `a08.multimodality_probe.sampling` odnotowuje, że
najniżej numerowane planety mają trace w **52 %** przypadków wobec **16 %** globalnie, czyli dostępność
trace'u jest skorelowana z numerem planety. „Wszystkie, które mają trace" jest więc prawdą, ale **nie
odpowiada na pytanie o reprezentatywność** — porównanie rozkładów cech między tymi 663 i pozostałymi
3480 wierszami jest jednorazowym, tanim pomiarem, którego nie wykonano:
...
Zastrzeżenie do tej jednej liczby: oba ramiona eksperymentu warunkowania to
`HistGradientBoostingRegressor(max_iter=250, random_state=0)` na 12 000 wierszach splitu treningowego ADC
(`a15:217,236-237` i `:187-193,211,327`; wielkość próby jest też w payloadzie, `sampling.adc_n_train`), **nie ExoBiome**. Punkt odniesienia 0.4354 jest o 45 % gorszy
od 0.2994, więc to model o innej wrażliwości na dodatkowe cechy. Wniosek „T jest w praktyce wejściem"
opiera się na trzech pomiarach (`r = 0,9928`, RMSE 48,5 K, GBM z aux 62,8 K), które są od tego niezależne;
sama liczba 0,3 % dotyczy zastępnika. Ma to konsekwencję poza K9: **ta sama liczba 0,259 % jest jedynym ilościowym zakotwiczeniem progu
0,20** w drabinie `a26` (`a26:59-86`, `threshold.derived_from`), więc próg dziedziczy to zawężenie.
Obok zakotwiczenia kod podaje jeszcze dwustronny argument o rozdzielczości (`a26:67-68`: próg 0,05
„lapalby ten sam fizyczny efekt i dawalby FAIL za poprawna fizyke", a 0,50 „przepuszczalby model
bioracy polowe skillu z katalogu gwiazd"), więc wybór 0,20 nie jest czysto arbitralny — ale kalibracja
ilościowa stoi na jednym pomiarze na zastępniku. Mocniejsze zastrzeżenie zgłasza sam check:
`threshold.binding_today = false`, bo zmierzone udziały (−0,002 na ADC, −0,004 na tau-val) leżą
50–100× pod progiem i mają **przeciwny znak**, więc **zdolność rozdzielcza progu przy granicy nie jest
na tych danych przetestowana** (`a26:70-73`).
...
**Dowód na wersji generującej (1.3.2).** Rozstrzygające jest to, że atmosfera zbudowana przez
`make_atmosphere` **na przypiętym POSEIDON-ie 1.3.2** — czyli na tej wersji, która wygenerowała zbiór
(`manifest.json: software_versions.poseidon`) — zawiera tablicę mieszanin CIA:
`reports/audit/d01_poseidon_132/d01_poseidon_diagnosis.json`,
`payload.stage1.grids.repo_increasing.atmosphere_keys` obejmuje **`X_CIA`** obok `X`, `X_active`,
`X_bf`, `X_ff`. Nasze wywołanie nie prosi o CIA i nie potrafi go wyłączyć, a mimo to biblioteka sama
alokuje kanał CIA — CIA jest więc po stronie POSEIDON-a częścią konstrukcji modelu, nie opcją.

*Uzupełniająco, lektura źródła na 1.4.0.* Mechanizm doboru par widać wprost w kodzie zainstalowanego
pakietu; **ta lektura jest na 1.4.0, nie na wersji generującej**, więc stoi tu jako wyjaśnienie
mechanizmu, a nie jako dowód o zbiorze (dowodem jest `X_CIA` z 1.3.2 wyżej):
...
tego nie wyłącza; `extinction(...)` dodaje CIA do κ zawsze, gdy `CIA_pairs` niepuste. Liczby 6 par
i wartości domyślnej `disable_continuum` **nie zweryfikowano na 1.3.2** — patrz „co by to obaliło".
...
*Kotwica B — źródło TauREx3, wywołanie odtworzone.* **Zastrzeżenie o wersji, bo jest tu asymetria
wobec kotwicy A:** zbiór tau wygenerowano `taurex` **3.2.4** (`manifest.json: software_versions.tau`,
pin w `requirements-generator-taurex.txt`), a jedyną instalacją TauREx-a w repo jest **3.3.2**
(`.venv-cnn`) — więc poniższa lektura jest na wersji **o jedną minor wyżej niż generująca** i nie jest
pomiarem na 3.2.4. Kotwica A (oficjalny FM organizatorów) jest od wersji niezależna i to na niej stoi
ustalenie; kotwica B tylko wyklucza domyślne CIA w bibliotece. W `taurex 3.3.2` lista wkładów startuje
...
| | |
|---|---|
| check | **brak checku**, weryfikacja przez **lekturę trzech źródeł spoza audytu**: `data/crossgen_biosignatures/taurex_backend.py:148,154-157` (nasze wywołanie — jedyne w pliku), `models/adc_baseline/FM_utils_final.py:183,207-209` (oficjalny FM ADC2023 organizatorów — trzy wkłady + `CIACache`; kotwica **niezależna od wersji**), oraz `taurex/model/model.py:42,57-65` + `simplemodel.py:121-123` w zainstalowanym `taurex 3.3.2` (brak domyślnego CIA) — **z zastrzeżeniem, że zbiór wygenerowano `taurex 3.2.4`, a 3.2.4 nie jest w repo zainstalowana**, więc kotwica B jest lekturą o minor wyżej. Po stronie POSEIDON-a dowód jest na wersji generującej: `X_CIA` w `atmosphere_keys` rekordu **1.3.2** (`reports/audit/d01_poseidon_132/d01_poseidon_diagnosis.json`) |
...
| co by to obaliło | **(1)** Wystąpienie `CIAContribution` (albo równoważnego wkładu CIA) w liście `contributions` w `data/crossgen_biosignatures/` — wtedy generator jednak liczy kontinuum i ustalenie upada w całości. **(2)** Wykazanie, że `taurex` dokłada CIA **implicite**, mimo pustej listy — wystarczy jeden przypadek, w którym widmo policzone bez `CIAContribution` różni się od widma z nim; lektura `taurex/model/model.py:42`, `:57-65` i `simplemodel.py:121-123` tego nie potwierdza, ale **to lektura na 3.3.2, nie pomiar na 3.2.4**: instalacja 3.2.4 i powtórzenie tej lektury jest najtańszym sposobem domknięcia tej luki. **(3)** Odtworzenie zapisanego widma tau **bez** CIA z niezerową rozbieżnością — pokazałoby, że zapisane widma nie powstały z tej konfiguracji. **(4)** Po stronie POSEIDON-a: brak `X_CIA` w atmosferze zbudowanej na 1.3.2 (jest — patrz rekord) albo wykazanie, że `CIA_pairs` w 1.3.2 wychodzi puste dla naszych `chemical_species`; liczby „6 par" i `disable_continuum = False` są odczytane z 1.4.0 i na 1.3.2 **niesprawdzone**. Żaden z tych warunków nie jest dziś spełniony |
| rekord | `—` |

---

## 7. Inwentarz plików potrzebnych do udowodnienia i naprawienia zarzutów
...
audit/a02_trivial_baseline.py        podłoga stałej dla raportowanych liczb — §0a i Załącznik A
...
audit/a08_reference_posterior.py     referencyjny nested sampling + sonda multimodalności
                                     — cytowane wyłącznie w Załączniku A
audit/a09_noise_realization.py       statystyka R z kalibracją na noiseless/noisy — K3 i Załącznik A
audit/a10_split_integrity.py         `taurex_fmpe`: holdout ≡ validation; brak testu in-domain na TauREx
audit/a11_pairing_audit.py           parowanie porównań — 19 różniących się pól konfiguracji quantum
                                     vs noquant, z czego jedno zadeklarowane; cytowane w §0a
audit/a12_significance_power.py      MDE i liczba seedów — Załącznik A
audit/a13_provenance_index.py        indeks prowenienacji artefaktów — §0a, K10(b), Załącznik A
audit/a14_importability.py           martwe importy — cytowane wyłącznie w Załączniku A
audit/a15_target_completeness.py     K9   (sprzężenie T-abundancje, wyciek T przez aux)
audit/a21_dead_features.py           martwe cechy + skład tła H2/He — K11
audit/a24_official_metrics.py        K10  (metryki ADC2022/ADC2023)
audit/a26_baseline_ladder.py         drabina stała/aux/widmo — §0a i Załącznik A
audit/a27_pipeline_fidelity.py       wierność rekonstrukcji pipeline'u — §1 (margines 11×)
audit/a29_smoke_baseline_recovery.py K1(b) — dopisane PO przebiegu 2026-07-27
audit/d01_poseidon_diagnosis.py      K1(c). W suite wołane z `--stage 0`, które nie wymaga bazy opacity.
                                     Etap 1 (`--stage 1`), na którym stoi K1(c), wymaga POSEIDON-a —
                                     nie ma go ani w .venv-qml, ani w .venv-cnn, więc idzie z osobnego
                                     venva, komenda w audit/README.md
```

Wszystkie 21 pozycji są w `run_all.SUITE`.
...
| ridge / GBM na POSEIDON (tylko-widmo) | skill **−0,019 / −0,127** — wspólny znak; MDE na n=685 to **0,0436** (`a12`), więc **−0,019 (ridge) leży pod progiem wykrywalności**, nieodróżnialne od zera; −0,127 (GBM) i −0,111 (ExoBiome) leżą nad nim | `a26`, `a12` |
...
| MDE (80 %, α=0.05): n=685 / n=64 | **0.0439** / **0.1453** (estymand per-wiersz) | `a12`; n=685 liczone na pasującym zbiorze, **n=64 jest orientacyjne** — poprawna arytmetyka na pożyczonym sd z 685 wierszy POSEIDON, bo dla komórek `data_64` repo nie ma predykcji per wiersz (`a12.mde_sd_source`) |
...
| ↳ dlaczego te listy się różnią: `a02` pyta, czy obok cytowanej liczby leży plik predykcji; `a13` pyta, czy claim ma **gdziekolwiek** wagi/metryki/kod. CNN 0,6500 jest dlatego w liście `a02`, ale **nie** w liście `a13` (tam `backed_summary`: wagi tak, metryki tak, predykcje NIE) — patrz K10(b) | — | `a02`, `a13` |
