...
export default function ModelsPage() {
  ...
  return (
    <div ref={ref} className="max-w-[900px] mx-auto px-4 py-6 space-y-6 cell-stagger">
      ...
      {/* Model cards — free flowing */}
      <div className="reveal space-y-4">
        <div className="font-mono text-[12px] text-nb-muted">
          # 3 models available
        </div>
        {models.map((m, i) => (
          <div key={i} className="border border-nb-border rounded-lg bg-nb-cell overflow-hidden">
            <div className={`flex items-center gap-2 px-4 py-2.5 border-b border-nb-border ${
              m.type === "quantum" ? "bg-indigo-50" : "bg-zinc-50"
            }`}>
              {m.type === "quantum" ? (
              ...
              <span className="font-mono text-[14px] font-semibold text-nb-text">{m.name}</span>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="bg-nb-card rounded-lg p-3 border border-nb-border">
      ...
      {/* Comparison table — free flowing */}
      <div className="reveal">
        <div className="font-mono text-[12px] text-nb-muted mb-3">
          # Model comparison table
        </div>
        <div className="overflow-x-auto border border-nb-border rounded-lg bg-nb-cell">
          <table className="w-full text-[13px] font-mono">
            <thead>
              <tr className="bg-nb-card border-b border-nb-border">
                <th className="text-left px-3 py-2.5 font-semibold text-nb-text">Metric</th>
                <th className="text-left px-3 py-2.5 font-semibold text-nb-blue">
                ...
                <th className="text-left px-3 py-2.5 font-semibold text-nb-muted">
            ...
            <tbody>
              {comparisonRows.map((row, i) => (
                <tr key={i} className={`border-b border-nb-border last:border-b-0 ${i % 2 === 0 ? "" : "bg-nb-card/50"}`}>
        ...
        <div className="mt-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg text-[12px] text-nb-text">
