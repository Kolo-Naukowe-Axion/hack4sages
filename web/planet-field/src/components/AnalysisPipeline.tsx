...
export function AnalysisPipeline({ active, onComplete }: Props) {
  ...
  // Fire onComplete when all stages done
  useEffect(() => {
  ...
  // Auto-advance non-circuit stages
  useEffect(() => {
    if (!active || currentStage < 0 || currentStage >= STAGES.length) return;
    if (currentStage === 2) return; // circuit stage handled by callback
  ...
  // Advance from circuit stage when circuit animation completes
  useEffect(() => {
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-6">
            ...
            >
              ...
              <div className="min-w-0">
                <div className={`text-sm font-medium ${
                  ...
                  {stage.label}
                  {isDone && <span className="text-accent-green ml-2">✓</span>}
                </div>
      ...
      {/* Quantum circuit visualization — appears during stage 2 */}
      <AnimatePresence>
        {currentStage === 2 && (
          <motion.div
