...
PREPARED_DATA="${PREPARED_DATA:-$PROJECT_ROOT/data/generated-data/taurex_fmpe_prepared}"
RUN_DIR="${RUN_DIR:-$PROJECT_ROOT/local_runs/taurex_fmpe_rtx4090_$(date +%Y%m%d_%H%M%S)}"
SETTINGS_FILE="${SETTINGS_FILE:-$WORKFLOW_DIR/settings/taurex_rtx4090.yaml}"
VENV_DIR="${VENV_DIR:-$PROJECT_ROOT/.venv-taurex-fmpe}"
...
python -m pip install --index-url "$PYTORCH_INDEX_URL" "$TORCH_PACKAGE_SPEC"
...
if [[ "${PREPARED_FORCE_OVERWRITE:-0}" == "1" || ! -f "$PREPARED_DATA/manifest.json" ]]; then
  python -m models.taurex_fmpe.prepare_dataset \
    --data-root "$DATA_ROOT" \
...
if [[ -f "$RUN_DIR/best_model_by_mrmse.pt" ]]; then
  python -u -m models.taurex_fmpe.evaluate \
    --run-dir "$RUN_DIR" \
    --prepared-data "$PREPARED_DATA"
  exit 0
fi

python -u -m models.taurex_fmpe.train \
