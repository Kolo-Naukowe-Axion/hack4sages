...
resolve_output_root() {
  ...
  ls -td "\$REMOTE_ROOT"/outputs/five_qubit_exobiome_two_stage_* 2>/dev/null | head -n 1
}
...
case "\$MODE" in
  ...
  tail)
    ...
    if [[ -z "\$output_root" ]]; then
      echo "No five_qubit_exobiome run directory found under \$REMOTE_ROOT/outputs" >&2
      exit 1
    fi
  ...
  status)
    ...
    ;;
  *)
    echo "Usage: monitor_five_qubit_exobiome_remote.sh [tail|attach|status]" >&2
    exit 1
    ;;
esac
EOF
