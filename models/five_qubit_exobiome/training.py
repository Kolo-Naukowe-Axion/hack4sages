"""Training loop for the Five-qubit ExoBiome Ariel hybrid regressor."""
...
def resolve_training_device(config: TrainingConfig) -> torch.device:
    ...
    if config.classical_only and getattr(torch.backends, "mps", None) is not None and torch.backends.mps.is_available():
