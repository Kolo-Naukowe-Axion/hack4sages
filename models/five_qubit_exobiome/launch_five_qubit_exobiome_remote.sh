...
ENV_DIR="${ENV_DIR:-/home/iwo/.venvs/five-qubit-exobiome}"
PYTHON_BIN="${PYTHON_BIN:-$ENV_DIR/bin/python}"
DATA_ROOT="${DATA_ROOT:-$REMOTE_ROOT/data/ariel-ml-dataset}"
SESSION_NAME="${SESSION_NAME:-five_qubit_exobiome}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$REMOTE_ROOT/outputs/five_qubit_exobiome_two_stage_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -x "\$PYTHON_BIN" ]]; then
  ...
  echo "  ENV_DIR=\$ENV_DIR PYTHON_BIN=python3.11 bash \$REMOTE_ROOT/models/five_qubit_exobiome/setup_five_qubit_exobiome_env.sh" >&2
  exit 1
fi
...
tmux new-session -d -s "\$SESSION_NAME" \
  "cd \$REMOTE_ROOT && PYTHON_BIN=\$PYTHON_BIN bash models/five_qubit_exobiome/run_five_qubit_exobiome_two_stage.sh \$DATA_ROOT \$OUTPUT_ROOT"
