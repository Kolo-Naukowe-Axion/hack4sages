...
rsync -a --delete --info=progress2 -e "ssh ${REMOTE_SSH_ARGS}" \
  "${PROJECT_ROOT}/models/taurex_exobiome_without_quant/" \
  "${REMOTE_HOST}:${REMOTE_ROOT}/models/taurex_exobiome_without_quant/"

rsync -a --delete --info=progress2 -e "ssh ${REMOTE_SSH_ARGS}" \
  "${PROJECT_ROOT}/data/TauREx set/" \
  "${REMOTE_HOST}:${REMOTE_DATA_ROOT}/"

echo "Synced quantum model -> ${REMOTE_HOST}:${REMOTE_ROOT}/models/taurex_exobiome_without_quant"
