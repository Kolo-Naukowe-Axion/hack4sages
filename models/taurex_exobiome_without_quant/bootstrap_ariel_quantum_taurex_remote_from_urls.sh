...
SRC_URL="${SRC_URL:?Set SRC_URL to a tarball containing models/taurex_exobiome_without_quant.}"
...
rm -rf "$REMOTE_ROOT/models/taurex_exobiome_without_quant"
...
download_file "$SRC_URL" /tmp/taurex_exobiome_without_quant_remote.tgz
download_file "$LABELS_URL" "$REMOTE_DATA_ROOT/labels.parquet"
download_file "$SPECTRA_URL" "$REMOTE_DATA_ROOT/spectra.h5"
tar -xzf /tmp/taurex_exobiome_without_quant_remote.tgz -C "$REMOTE_ROOT"
...
if command -v tmux >/dev/null 2>&1; then
  ...
  tmux new-session -d -s "$SESSION_NAME" \
    "cd $REMOTE_ROOT && DATA_ROOT=$(printf '%q' "$REMOTE_DATA_ROOT") OUTPUT_ROOT=$(printf '%q' "$OUTPUT_ROOT") ENV_DIR=$(printf '%q' "$ENV_DIR") PYTHON_BIN=$(printf '%q' "$PYTHON_BIN") UPGRADE_PIP=$(printf '%q' "$UPGRADE_PIP") bash models/taurex_exobiome_without_quant/run_ariel_quantum_taurex_ubuntu_gpu.sh"
