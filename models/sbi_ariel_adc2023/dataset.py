"""Dataset loading for prepared ADC2023 five-gas FMPE arrays."""
...
class NormalizationStats:
    ...
    def load(cls, prepared_dir: Path) -> "NormalizationStats":
        ...
        if manifest.get("dataset_type") != DATASET_TYPE or manifest.get("normalization_mode") != NORMALIZATION_MODE:
            raise RuntimeError(
                "Prepared dataset normalization does not match the ADC2023 five-gas FMPE adapter. "
...
class ADC2023FiveGasPreparedDataset(Dataset):
    """Prepared labeled split for ADC2023 five-gas FMPE training."""
    ...
    def sample_ids(self) -> np.ndarray:
        return self.metadata["planet_ID"].to_numpy(dtype="U32")


class ADC2023FiveGasInferenceDataset:
    """Prepared unlabeled test split for ADC2023 five-gas FMPE inference."""
...
def load_datasets(
    ...
    datasets = {
        "train": ADC2023FiveGasPreparedDataset(prepared_dir, settings["dataset"].get("train_split", TRAIN_SPLIT)),
        "validation": ADC2023FiveGasPreparedDataset(
            prepared_dir, settings["dataset"].get("validation_split", VALIDATION_SPLIT)
        ),
        "holdout": ADC2023FiveGasPreparedDataset(prepared_dir, settings["dataset"].get("holdout_split", HOLDOUT_SPLIT)),
        "testdata": ADC2023FiveGasInferenceDataset(prepared_dir),
    }
