...
ENV_DIR="${ENV_DIR:-$HOME/.venvs/five-qubit-codex}"
...
$ENV_DIR/bin/python -u $REPO_ROOT/models/five_qubit_codex/run_five_qubit_codex.py \\
  --data-root $REPO_ROOT/data/ariel-ml-dataset \\
  --output-dir $REPO_ROOT/outputs/five_qubit_codex_live \\
  --batch-size 64 \\
  --eval-batch-size 128 \\
  --max-epochs 30 \\
...
$ENV_DIR/bin/python -u $REPO_ROOT/models/five_qubit_codex/run_five_qubit_codex.py \\
  --data-root $REPO_ROOT/data/ariel-ml-dataset \\
  --output-dir $REPO_ROOT/outputs/five_qubit_codex_classical_mac \\
  --classical-only \\
  --batch-size 128 \\
  --eval-batch-size 256 \\
  --max-epochs 30 \\
