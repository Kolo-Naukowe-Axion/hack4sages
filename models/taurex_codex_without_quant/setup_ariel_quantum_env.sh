...
$ENV_DIR/bin/python -u $REPO_ROOT/models/taurex_codex_without_quant/run_ariel_quantum_regression.py \\
  --data-root "$REPO_ROOT/data/TauREx set" \\
  --dataset-format taurex \\
  --output-dir $REPO_ROOT/outputs/taurex_codex_without_quant_live \\
  --batch-size 64 \\
  --eval-batch-size 128 \\
  --max-epochs 30 \\
...
$ENV_DIR/bin/python -u $REPO_ROOT/models/taurex_codex_without_quant/run_ariel_quantum_regression.py \\
  --data-root "$REPO_ROOT/data/TauREx set" \\
  --dataset-format taurex \\
  --output-dir $REPO_ROOT/outputs/taurex_codex_without_quant_classical_mac \\
  --classical-only \\
  --batch-size 128 \\
  --eval-batch-size 256 \\
  --max-epochs 30 \\
