...
if command -v tmux >/dev/null 2>&1; then
  ...
  tmux new-session -d -s "$SESSION_NAME" \
    "cd $REMOTE_ROOT && DATA_ROOT=$(printf '%q' "$DATA_ROOT") OUTPUT_ROOT=$(printf '%q' "$OUTPUT_ROOT") ENV_DIR=$(printf '%q' "$ENV_DIR") bash models/taurex_codex_without_quant/run_ariel_quantum_taurex_ubuntu_gpu.sh"
