...
PACKAGE_NAME = "models.12q_taurex_exobiome"
MODEL_NAME = f"{PACKAGE_NAME}.model"
TRAINING_NAME = f"{PACKAGE_NAME}.training"
RUN_NAME = f"{PACKAGE_NAME}.run_12q_taurex_exobiome"
RUN_CV_NAME = f"{PACKAGE_NAME}.run_12q_taurex_exobiome_cross_validation"
...
class PackageContractTests(unittest.TestCase):
    def test_default_configs_target_12_qubits(self) -> None:
        ...
        self.assertEqual(str(constants_module.DEFAULT_OUTPUT_DIR), "outputs/12q_taurex_exobiome")

        model_source = (PROJECT_ROOT / "models" / "12q_taurex_exobiome" / "model.py").read_text(encoding="utf-8")
        training_source = (PROJECT_ROOT / "models" / "12q_taurex_exobiome" / "training.py").read_text(encoding="utf-8")
    ...
    def test_cli_parsers_default_to_new_paths_and_12_qubits(self) -> None:
        ...
        self.assertEqual(run_args.output_dir, "outputs/12q_taurex_exobiome")
        self.assertEqual(run_args.qnn_qubits, 12)

        run_cv_args = run_cv_module.build_parser().parse_args([])
        self.assertEqual(run_cv_args.output_dir, "outputs/12q_taurex_exobiome_cv")
        self.assertEqual(run_cv_args.qnn_qubits, 12)

    def test_shell_helpers_reference_new_package(self) -> None:
        package_dir = PROJECT_ROOT / "models" / "12q_taurex_exobiome"

        taurex_script = (package_dir / "run_12q_taurex_exobiome_taurex_ubuntu_gpu.sh").read_text(encoding="utf-8")
        self.assertIn("models/12q_taurex_exobiome/run_12q_taurex_exobiome.py", taurex_script)
        self.assertIn("--qnn-qubits 12", taurex_script)
        self.assertIn("outputs/12q_taurex_exobiome_taurex_", taurex_script)

        sync_script = (package_dir / "sync_12q_taurex_exobiome_remote.sh").read_text(encoding="utf-8")
        self.assertIn("models/12q_taurex_exobiome/", sync_script)

        launch_script = (package_dir / "launch_12q_taurex_exobiome_remote.sh").read_text(encoding="utf-8")
        self.assertIn("run_12q_taurex_exobiome_taurex_ubuntu_gpu.sh", launch_script)
