...
WORKFLOW_ROOT = PROJECT_ROOT / "models" / "taurex_fmpe"
...
class WorkflowSmokeTests(unittest.TestCase):
    def test_launcher_references_local_package(self) -> None:
        script = (WORKFLOW_ROOT / "run_train_ubuntu4090.sh").read_text(encoding="utf-8")
        self.assertIn("models.taurex_fmpe.prepare_dataset", script)
        self.assertIn("models.taurex_fmpe.train", script)
        self.assertIn("models.taurex_fmpe.evaluate", script)
        self.assertIn("settings/taurex_rtx4090.yaml", script)

    def test_missing_dataset_files_raise_clear_error(self) -> None:
        from models.taurex_fmpe.prepare_dataset import prepare_dataset
        ...
        with tempfile.TemporaryDirectory() as temp_root, tempfile.TemporaryDirectory() as temp_out:
            ...
            self.assertIn("TauREx dataset is incomplete", str(ctx.exception))
    ...
    def test_load_datasets_from_minimal_prepared_dir(self) -> None:
        from models.taurex_fmpe.constants import (
            CONTEXT_DIM,
        ...
        from models.taurex_fmpe.dataset import load_datasets
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for split_name, rows in ((TRAIN_SPLIT, 4), (VALIDATION_SPLIT, 2), (HOLDOUT_SPLIT, 2)):
                ...
                pd.DataFrame(
                    {
                        "sample_id": [f"tau_{i}" for i in range(rows)],
                        "generator": ["tau"] * rows,
                        "split": ["train"] * rows,
                        "source_row_index": np.arange(rows),
                    }
                ).to_csv(root / METADATA_FILENAME_TEMPLATE.format(split_name=split_name), index=False)
            np.save(root / CONTEXT_FILENAME_TEMPLATE.format(split_name=TESTDATA_SPLIT), np.zeros((3, CONTEXT_DIM), dtype=np.float32))
            pd.DataFrame(
                {
                    "sample_id": [f"tau_test_{i}" for i in range(3)],
                    "generator": ["tau"] * 3,
                    "split": ["val"] * 3,
                    "source_row_index": np.arange(3),
                }
            ).to_csv(root / METADATA_FILENAME_TEMPLATE.format(split_name=TESTDATA_SPLIT), index=False)
...
@unittest.skipUnless(HAS_H5PY and HAS_PYARROW and DATA_ROOT.exists(), "real TauREx dataset plus parquet/h5py support is required")
class DatasetContractTests(unittest.TestCase):
    def test_prepare_dataset_subset_shapes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(manifest["context_dim"], 444)
            self.assertEqual(manifest["theta_dim"], 5)
            self.assertTrue(manifest["holdout_mirrors_validation"])
